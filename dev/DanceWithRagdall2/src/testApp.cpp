...
void testApp::setup()
{
    ...
    m_picker.setWorld(ragdollScene.getDynamicsWorld());
    if (ragdolls.size() != 0)
        m_picker.attatchRigidBody(ragdolls.at(0)->m_bodies[RagDoll::BODYPART_HEAD]);
}
...
void testApp::drawActor(ramActor &actor)
{
    ...
    ofVec3f vl = actor.getNode(ramActor::JOINT_LEFT_HAND).getPosition();
    m_handPos[LA].setValue(vl.x, vl.y, vl.z);
    
    ofVec3f vr = actor.getNode(ramActor::JOINT_RIGHT_HAND).getPosition();
    m_handPos[RA].setValue(vr.x, vr.y, vr.z);
    
    //ofVec3f vh = actor.getNode(ramActor::JOINT_HEAD).getPosition();
    //m_handPos[HD].setValue(vh.x, vh.y, vh.z);
    
    //ofVec3f vll = actor.getNode(ramActor::JOINT_LEFT_TOE).getPosition();
    //m_handPos[LL].setValue(vll.x, vll.y, vll.z);
    
    //ofVec3f vrl = actor.getNode(ramActor::JOINT_RIGHT_TOE).getPosition();
    //m_handPos[RL].setValue(vrl.x, vrl.y, vrl.z);
    m_handPos[RL].setValue(0.0f, 0.0f, 0.0f);
    
    const ofVec3f head = actor.getNode(ramActor::JOINT_HEAD).getPosition();
    m_picker.updatePosition(btVector3(head.x, head.y, head.z));
            ...
            if (ragdolls.size() != 0) {
                ...
                for (int j=0; j<ragdolls.size(); j++) {
                    ...
                    switch (i) {
                        ...
                        //    break;
                        //case LL:
                        //    part = RagDoll::BODYPART_LEFT_LOWER_LEG;
                        //    break;
                        case RL:
                            part = RagDoll::BODYPART_RIGHT_LOWER_LEG;
                            break;
